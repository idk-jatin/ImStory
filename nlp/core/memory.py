...
class World:
    ...
    def new_ent(self, name, kind, mention, page, embedding=None):
        self.counter += 1
        node = Entity(self.counter, name, kind, embedding)
        node.update(mention, page, embedding)
        self.ents[self.counter] = node
        return node
    ...
    def sim_ent(self, name, embedding=None):
        ...
        for ent in self.ents.values():
            ...
            for a in ent.aliases:
                a_head = normalize(a).split()[-1]
                if a_head == name_head:
                    return ent
            if embedding is not None and ent.mean_emb() is not None:
                sim = cosine_similarity([embedding], [ent.mean_emb()])[0][0]
                if sim >= self.sim_ts:
                    return ent
